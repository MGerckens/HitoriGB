...
static void swap_rows(uint8_t row1, uint8_t row2){
	set_bkg_tile_xy(1,1,0);
    uint8_t *temp = malloc(board_size * sizeof(uint8_t));
	memcpy(temp, board+row1, board_size);
    memcpy(board+row1, board+row2, board_size);
	memcpy(board+row2,temp,board_size);
	free(temp);
